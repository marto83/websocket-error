...
namespace KestrelWebSocketServer
{
    public class WebSocketMiddleware : IMiddleware
    {
        ...
        private static ConcurrentDictionary<int, ConnectedClient> Clients =
            ...
            try
            {
                if (ServerIsRunning)
                {
                    if (context.WebSockets.IsWebSocketRequest)
                    {
                        ...
                        client.BroadcastQueue.Add(File.ReadAllText("Responses/Connected.json"));
        ...
        private static async Task SocketProcessingLoopAsync(ConnectedClient client)
        {
            ...
            _ = Task.Run(() => client.ReceiveLoopAsync().ConfigureAwait(false));
            ...
            try
            {
                       ...
                       !loopToken.IsCancellationRequested)
                {
                    using (var ms = new MemoryStream())
                    {
                        ...
                        {
                            if (await HandleCloseMessage(receiveResult))
                                return;
                            ...
                            if (client.Socket.State == WebSocketState.Open)
                            {
                                ms.Write(
                                    ...
                                    buffer.Offset, receiveResult.Count);
                                ...
                                switch (messageType)
                                {
                                    case WebSocketMessageType.Text:
                                        ...
                                        client.ReceiveQueue.Add(message);
                                        break;
                                    case WebSocketMessageType.Binary:
                                        throw new InvalidOperationException("Do not support binary messages");
                                }
                            }
                        }
                    }
            ...
            async Task<bool> HandleCloseMessage(WebSocketReceiveResult receiveResult)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine(
                            $"Socket {client.SocketId}: Acknowledging Close frame received from client");
                        broadcastTokenSource.Cancel();
                ...
                {
                    Console.WriteLine("Error handling close message: " + e.Message);
                    return false;
                }
            }
        }
